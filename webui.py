...
from llm import init_models, init_vector_store
from data_processing import load_and_validate_json_files, create_nodes
...
def disable_streamlit_watcher():
    """Patch Streamlit to disable file watcher. Safe to call when runtime not created."""
    try:
        ...
        if hasattr(inst, "_on_script_changed"):
            inst._on_script_changed = lambda _ : None
    except Exception:
        return
...
def main():
    ...
    prompt = st.chat_input("请输入劳动法相关问题")
    if prompt:
        # 添加用户消息到历史
